...
_pseudocyl = ["moll", "kav7", "eck4", "robin",
    ...
    * __call__ method compute transformations.
    ...
    def __init__(self, projparams,  # pylint: disable=too-many-arguments
        ...
        if self.projection == "cyl":
            ...
            _x1, y1 = projtmp(
                0., latmax)  # pylint: disable=unpacking-non-sequence
            x2, _y2 = projtmp(
                lonmax, 0.)  # pylint: disable=unpacking-non-sequence
            width, height = x2, y1
            self._height = height
            self._width = width
            if llcrnrlon == -180 and llcrnrlat == -90 and urcrnrlon == +180 and urcrnrlat == +90:
    ...
    def __call__(self, *args, **kw):
        ...
        if inverse:
            if self.projection in ["merc", "mill", "gall"]:
                ...
                rcurv = self.rmajor * coslat / \
                    math.sqrt(1. - self.esq * sinlat**2)
                if onearray:
                    ...
                    try:
                        ...
                        # x is a sequence.
                        outx = [_rad2dg * (xi / rcurv) +
                                self.llcrnrlon for xi in x]
        else:
            if self.projection in ["merc", "mill", "gall"]:
                if self.projection == "merc":
                    coslat = math.cos(math.radians(self.projparams["lat_ts"]))
    ...
    def makegrid3d(self, nx, ny, returnxy=False):
        ...
        xy[..., 0] = self.llcrnrx + dx * \
            np.indices((ny, nx), np.float32)[1, :, :]
        xy[..., 1] = self.llcrnry + dy * \
            np.indices((ny, nx), np.float32)[0, :, :]
