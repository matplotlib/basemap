...
_pseudocyl = ['moll', 'robin', 'eck4', 'kav7',
              'sinu', 'mbtfpq', 'vandg', 'hammer']
    ...
    def with_transform(self, x, y, data, *args, **kwargs):
        ...
        if kwargs.pop('latlon', latlon_default):
            ...
            if self.projection in _cylproj or self.projection in _pseudocyl:
                x, data = self.shiftdata(
                    x, data, fix_wrap_around=plotfunc.__name__ not in ["scatter"])
    ...
    def with_transform(self, x, y, *args, **kwargs):
        ...
        if kwargs.pop('latlon', latlon_default):
            ...
            if self.projection in _cylproj or self.projection in _pseudocyl:
                if x.ndim == 1:
                    x = self.shiftdata(
                        x, fix_wrap_around=plotfunc.__name__ not in ["scatter"])
                elif x.ndim == 0:
                    if x > 180:
                        x = x - 360.
            # convert lat/lon coords to map projection coords.
...
class Basemap(object):
    ...
    def __init__(self, llcrnrlon=None, llcrnrlat=None,
        ...
        using_corners = (
            None not in [llcrnrlon, llcrnrlat, urcrnrlon, urcrnrlat])
        if using_corners:
        ...
        if projection in ['lcc', 'eqdc', 'aea']:
            ...
            if lat_1 is None or lon_0 is None:
                raise ValueError(
                    'must specify lat_1 or lat_0 and lon_0 for %s basemap (lat_2 is optional)' %
                    _projnames[projection])
            if lat_2 is None:
            ...
            if not using_corners:
                ...
                if using_cornersxy:
                    llcrnrlon, llcrnrlat, urcrnrlon, urcrnrlat = _choosecornersllur(
                        llcrnrx, llcrnry, urcrnrx, urcrnry, **projparams)
                    ...
                    if width is None or height is None:
                        raise ValueError(
                            'must either specify lat/lon values of corners (llcrnrlon,llcrnrlat,urcrnrlon,urcrnrlat) in degrees or width and height in meters')
                    if lon_0 is None or lat_0 is None:
                        raise ValueError(
                            'must specify lon_0 and lat_0 when using width, height to specify projection region')
                    llcrnrlon, llcrnrlat, urcrnrlon, urcrnrlat = _choosecorners(
                        width, height, **projparams)
                    self.llcrnrlon = llcrnrlon
                    self.llcrnrlat = llcrnrlat
                    self.urcrnrlon = urcrnrlon
                    self.urcrnrlat = urcrnrlat
            ...
            if lat_0 is None or lon_0 is None:
                raise ValueError(
                    'must specify lat_0 and lon_0 for Stereographic basemap (lat_ts is optional)')
            if not using_corners:
                if width is None or height is None:
                    raise ValueError(
                        'must either specify lat/lon values of corners (llcrnrlon,llcrnrlat,urcrnrlon,urcrnrlat) in degrees or width and height in meters')
            ...
            if (projection == 'splaea' and boundinglat >= 0) or\
               (projection == 'nplaea' and boundinglat <= 0):
                raise ValueError(
                    'boundinglat cannot extend into opposite hemisphere')
            if boundinglat is None or lon_0 is None:
                raise ValueError(
                    'must specify boundinglat and lon_0 for %s basemap' %
                    _projnames[projection])
            if projection[0] == 's':
                sgn = -1
            else:
            ...
            if width is not None or height is not None:
                sys.stdout.write(
                    'warning: width and height keywords ignored for %s projection' %
                    _projnames[projection])
        elif projection == 'laea':
            if lat_0 is None or lon_0 is None:
                raise ValueError(
                    'must specify lat_0 and lon_0 for Lambert Azimuthal basemap')
            if not using_corners:
                if width is None or height is None:
                    raise ValueError(
                        'must either specify lat/lon values of corners (llcrnrlon,llcrnrlat,urcrnrlon,urcrnrlat) in degrees or width and height in meters')
            ...
            if projection == 'gnom' and 'R' not in projparams:
                raise ValueError(
                    'gnomonic projection only works for perfect spheres - not ellipsoids')
            if lat_0 is None or lon_0 is None:
                raise ValueError(
                    'must specify lat_0 and lon_0 for Transverse Mercator, Gnomonic, Cassini-Soldnerr and Polyconic basemap')
            if not using_corners:
                if width is None or height is None:
                    raise ValueError(
                        'must either specify lat/lon values of corners (llcrnrlon,llcrnrlat,urcrnrlon,urcrnrlat) in degrees or width and height in meters')
            ...
            if 'R' not in projparams:
                raise ValueError(
                    'orthographic projection only works for perfect spheres - not ellipsoids')
            if lat_0 is None or lon_0 is None:
                raise ValueError(
                    'must specify lat_0 and lon_0 for Orthographic basemap')
            ...
            if width is not None or height is not None:
                sys.stdout.write(
                    'warning: width and height keywords ignored for %s projection' % _projnames[self.projection])
            if not using_corners:
                llcrnrlon = -180.
                llcrnrlat = -90.
                urcrnrlon = 180
            ...
            if np.abs(lat_0) < 1.e-2:
                lat_0 = 1.e-2
            projparams['lat_0'] = lat_0
        elif projection == 'geos':
            if lat_0 is not None and lat_0 != 0:
                raise ValueError(
                    'lat_0 must be zero for Geostationary basemap')
            if lon_0 is None:
                raise ValueError(
                    'must specify lon_0 for Geostationary basemap')
            if width is not None or height is not None:
                sys.stdout.write(
                    'warning: width and height keywords ignored for %s projection' % _projnames[self.projection])
            if not using_corners:
            ...
            if 'R' not in projparams:
                raise ValueError(
                    'near-sided perspective projection only works for perfect spheres - not ellipsoids')
            if lat_0 is None or lon_0 is None:
                raise ValueError(
                    'must specify lon_0 and lat_0 for near-sided perspective Basemap')
            if width is not None or height is not None:
                sys.stdout.write(
                    'warning: width and height keywords ignored for %s projection' % _projnames[self.projection])
            if not using_corners:
            ...
            if lat_1 is None or lon_1 is None or lat_2 is None or lon_2 is None:
                raise ValueError(
                    'must specify lat_1,lon_1 and lat_2,lon_2 for Oblique Mercator basemap')
            projparams['lat_1'] = lat_1
            ...
            if no_rot:
                projparams['no_rot'] = ''
            # if not using_corners:
            # raise ValueError, 'cannot specify map region with width and height
            # keywords for this projection, please specify lat/lon values of corners'
            if not using_corners:
                if width is None or height is None:
                    raise ValueError(
                        'must either specify lat/lon values of corners (llcrnrlon,llcrnrlat,urcrnrlon,urcrnrlat) in degrees or width and height in meters')
            ...
            if lat_0 is None or lon_0 is None:
                raise ValueError(
                    'must specify lat_0 and lon_0 for Azimuthal Equidistant basemap')
            if not using_corners:
                if width is None or height is None:
                    self._fulldisk = True
                    llcrnrlon = -180.
            ...
            if lon_0 is None or o_lon_p is None or o_lat_p is None:
                raise ValueError(
                    'must specify lon_0,o_lat_p,o_lon_p for rotated pole Basemap')
            if width is not None or height is not None:
                sys.stdout.write(
                    'warning: width and height keywords ignored for %s projection' % _projnames[self.projection])
            projparams['lon_0'] = lon_0
            ...
            if not using_corners and None in [llcrnrx, llcrnry, urcrnrx, urcrnry]:
                raise ValueError(
                    'must specify lat/lon values of corners in degrees')
            if None not in [llcrnrx, llcrnry, urcrnrx, urcrnry]:
                p = pyproj.Proj(projparams)
                llcrnrx = _dg2rad * llcrnrx
                llcrnry = _dg2rad * llcrnry
                urcrnrx = _dg2rad * urcrnrx
                urcrnry = _dg2rad * urcrnry
        ...
        # initialize proj4
        proj = Proj(projparams, self.llcrnrlon, self.llcrnrlat,
        ...
        atts = ['rmajor', 'rminor', 'esq',
                'flattening', 'ellipsoid', 'projparams']
        for att in atts:
        ...
        self.ymax = proj.ymax
        if projection == 'cyl':
            self.aspect = (self.urcrnrlat - self.llcrnrlat) / \
                (self.urcrnrlon - self.llcrnrlon)
        else:
    ...
    def _readboundarydata(self, name, as_polygons=False):
        ...
        if name != 'gshhs':
            as_polygons = False
        try:
            bdatfile = open(os.path.join(basemap_datadir, name +
                            '_' + self.resolution + '.dat'), 'rb')
            bdatmetafile = open(os.path.join(
                basemap_datadir, name + 'meta_' + self.resolution + '.dat'), 'r')
        except BaseException:
            raise IOError(
        ...
        if containsPole and\
            ...
            raise ValueError('%s projection cannot cross pole' %
                             (self.projection))
        ...
        tostere =\
            ['omerc', 'ortho', 'gnom', 'nsper',
                'nplaea', 'npaeqd', 'splaea', 'spaeqd']
        ...
        for line in bdatmetafile:
            ...
            crossdatelineE = False
            crossdatelineW = False
            if name == 'gshhs':
                id = linesplit[7]
                if id.endswith('E'):
                    crossdatelineE = True
                ...
                # projection coordinates.
                    ...
                    if name != 'gshhs' or (name == 'gshhs' and not as_polygons):
                        ...
                        if np.sum(split) and self.projection not in _cylproj:
                            ind = (np.compress(split, np.squeeze(
                                split * np.indices(xd.shape))) + 1).tolist()
                            iprev = 0
                            ind.append(len(xd))
                            for i in ind:
                                ...
                                if len(list(range(iprev, i))):
                                    polygons.append(
                                        list(zip(bx[iprev:i], by[iprev:i])))
                                iprev = i
                        else:
                            polygons.append(list(zip(bx, by)))
                        polygon_types.append(typ)
                    ...
                    if name == 'gshhs' and not as_polygons:
                        ...
                        # GEOS_ERROR: CGAlgorithmsDD::orientationIndex encountered
                        # NaN/Inf numbers
    ...
    def _getmapboundary(self):
        ...
        if self.projection in _cylproj:
            ...
            lons = [self.llcrnrlon, self.llcrnrlon,
                    self.urcrnrlon, self.urcrnrlon]
    ...
    def drawmapboundary(self, color='k', linewidth=1.0, fill_color=None,
        ...
        if self.projection in ['ortho', 'geos', 'nsper'] or (
            ...
            limb = Ellipse((self._width, self._height), 2. *
        ...
        if self.projection in ['ortho', 'geos', 'nsper', 'aeqd'] and self._fulldisk:
            ...
            lons1 = len(lats1) * [lon_0 - 179.9]
            # top.
            lons2 = np.linspace(lon_0 - 179.9999, lon_0 +
                                179.9999, nx).tolist()
            lats2 = len(lons2) * [89.9999]
            # right side
            lats3 = np.linspace(89.9999, -89.9999, ny).tolist()
            ...
            # bottom.
            lons4 = np.linspace(lon_0 + 179.9999, lon_0 -
                                179.9999, nx).tolist()
            lats4 = len(lons4) * [-89.9999]
            lons = np.array(lons1 + lons2 + lons3 + lons4, np.float64)
            lats = np.array(lats1 + lats2 + lats3 + lats4, np.float64)
            ...
            ax.set_frame_on(False)
            limb = Circle((0.5 * (self.xmax + self.xmin),
                           0.5 * (self.ymax + self.ymin)),
                          radius=0.5 * (self.xmax - self.xmin),
                          fc='none')
        else:  # all other projections are rectangular.
            ...
            if 1:
                ...
                if self.coastpolygontypes[npoly] not in [2, 4]:
                    ...
                    if lake_color is None:
                        poly = Polygon(xy, facecolor=axisbgc,
                                       edgecolor=axisbgc, linewidth=0)
                    else:
                        poly = Polygon(xy, facecolor=lake_color,
                                       edgecolor=lake_color, linewidth=0)
                if zorder is not None:
    ...
    def drawcoastlines(
        ...
        # get current axes instance (if none specified).
        ax = ax or self._check_ax()
        coastlines = LineCollection(
            self.coastsegs, antialiaseds=(antialiased,))
    ...
    def drawcounties(self, linewidth=0.1, linestyle='solid', color='k', antialiased=1,
        ...
        county_info = self.readshapefile(
            gis_file,
            'counties',
    ...
    def readshapefile(self, shapefile, name, drawbounds=True, zorder=None,
        ...
        info = (shf.numRecords, shptype,
                bbox[0:2] + [0., 0.], bbox[2:] + [0., 0.])
        npoly = 0
        for shprec in shf.shapeRecords():
            ...
            if shptype in [1, 8]:  # a Point or MultiPoint shape.
                ...
                if max(lons) > 721. or min(lons) < - \
                        721. or max(lats) > 90.01 or min(lats) < -90.01:
    ...
    def drawparallels(
        ...
        if self.projection in _cylproj + _pseudocyl:
            ...
            lonmin = self.boundarylonmin
            lonmax = self.boundarylonmax
        ...
        for circ in circlesl:
            ...
            testx = np.logical_and(
                x >= self.xmin - 3 * xdelta, x <= self.xmax + 3 * xdelta)
            x = np.compress(testx, x)
            y = np.compress(testx, y)
            testy = np.logical_and(
                y >= self.ymin - 3 * ydelta, y <= self.ymax + 3 * ydelta)
            ...
            if len(x) > 1 and len(y) > 1:
                ...
                for x, y in zip(xl, yl):
                    ...
                    if len(x) > 1 and len(y) > 1:
                        # Workaround for PDF rendering bug: avoid using linewidth=0
                        # directly.
        ...
        if self.projection in [
            ...
            if self.projection == 'vandg' or self._fulldisk:
                sys.stdout.write(
                    'Warning: Cannot label parallels on %s basemap' % _projnames[self.projection])
                labels = [0, 0, 0, 0]
            ...
            if side in ['l', 'r']:
                ...
                if side == 'l':
                    if self.projection in _pseudocyl:
                        ...
                        if self.celestial:
                            lons = (self.projparams['lon_0'] +
                                    180.) * np.ones(len(lats), lats.dtype)
                        else:
                            lons = (self.projparams['lon_0'] -
                                    180.) * np.ones(len(lats), lats.dtype)
                ...
                if max(lons) > 1.e20 or max(lats) > 1.e20:
                    raise ValueError(
                        'inverse transformation undefined - please adjust the map projection region')
                ...
                if side == 'b':
                    lons, lats = self(
                        xx, self.llcrnry * np.ones(xx.shape, np.float32), inverse=True)
                    lons = lons.tolist()
                    lats = lats.tolist()
                else:
                    lons, lats = self(
                        xx, self.urcrnry * np.ones(xx.shape, np.float32), inverse=True)
                    lons = lons.tolist()
                    lats = lats.tolist()
                if max(lons) > 1.e20 or max(lats) > 1.e20:
                    raise ValueError(
                    ...
                    if n >= 0:
                        ...
                        if side == 'l':
                            ...
                            t = ax.text(
                                ...
                                **text_kwargs)
                        if t is not None:
                            linecolls[lat][1].append(t)

        # set axes limits to fit map region.
        self.set_axes_limits(ax=ax)
        keys = list(linecolls.keys())
        vals = list(linecolls.values())
    ...
    def drawmeridians(
        ...
        if self.projection in _cylproj or self.projection in _pseudocyl:
            def addlon(meridians, madd):
                ...
                if minside and madd not in meridians:
                    meridians.append(madd)
                return meridians
        ...
        # don't draw meridians past latmax, always draw parallel at latmax.
        if latmax is None:
            latmax = 80.  # unused w/ cyl, merc or miller proj.
        # offset for labels.
        if yoffset is None:
            yoffset = (self.urcrnry - self.llcrnry) / 100.
            if self.aspect > 1:
        ...
        if self.projection in ['sinu', 'moll', 'hammer', 'vandg'] and max(labels):
            sys.stdout.write('Warning: Cannot label meridians on %s basemap' %
                             _projnames[self.projection])
            labels = [0, 0, 0, 0]
        ...
        for dolab, side in zip(labels, ['l', 'r', 't', 'b']):
            ...
            if side in ['l', 'r']:
                ...
                if side == 'l':
                    lons, lats = self(
                        self.llcrnrx * np.ones(yy.shape, np.float32), yy, inverse=True)
                    lons = lons.tolist()
                    lats = lats.tolist()
                else:
                    lons, lats = self(
                        self.urcrnrx * np.ones(yy.shape, np.float32), yy, inverse=True)
                    lons = lons.tolist()
                    lats = lats.tolist()
                if max(lons) > 1.e20 or max(lats) > 1.e20:
                    raise ValueError(
        ...
        if self.round:
            ...
            for merid in meridict:
                ...
                if pole == 1:
                    theta = (np.pi / 180.) * \
                        (merid - self.projparams['lon_0'] - 90)
                    if self.projection == 'ortho' and\
                       self.projparams['lat_0'] == -90:
                        theta = (np.pi / 180.) * (-merid +
                                                  self.projparams['lon_0'] + 90)
                    ...
                    if y > 0.5 * (self.ymin + self.ymax) + yoffset:
                        ...
                        vertalign = 'center'
                    # labels [l,r,t,b]
                    if labels[0] and not labels[1] and x >= 0.5 * \
                            (self.xmin + self.xmax) + xoffset:
                        continue
                    if labels[1] and not labels[0] and x <= 0.5 * \
                            (self.xmin + self.xmax) - xoffset:
                        continue
                    if labels[2] and not labels[3] and y <= 0.5 * \
                            (self.ymin + self.ymax) - yoffset:
                        continue
                    if labels[3] and not labels[2] and y >= 0.5 * \
                            (self.ymin + self.ymax) + yoffset:
                        continue
                elif pole == -1:
                    theta = (np.pi / 180.) * (-merid +
                    ...
                    if labels[3] and not labels[2] and y <= 0.5 * \
                            (self.ymin + self.ymax) + yoffset:
                        continue
                t = ax.text(x, y, lonlab, horizontalalignment=horizalign,
                            verticalalignment=vertalign, **text_kwargs)
    ...
    def drawgreatcircle(self, lon1, lat1, lon2, lat2, del_s=100., **kwargs):
        ...
        # since we know the difference between any two points, we can use this to
        # find wrap arounds on the plot
        ...
        for i, k in enumerate(cuts):
            ...
            # create new vertices with a nan inbetween and set those as the path's
            # vertices
    ...
    def transform_scalar(
        ...
        if self.projection not in _cylproj:
            ...
            if count > 1:
                raise ValueError(
                    'grid must be shifted so that lons are monotonically increasing and fit in range -180,+180 (see shiftgrid function)')
        ...
        datout = interp(datin, lons, lats, lonsout, latsout,
                        checkbounds=checkbounds, order=order, masked=masked)
        if returnxy:
            return datout, x, y
        else:
            return datout
    ...
    def set_axes_limits(self, ax=None):
        ...
        # if map boundary not yet drawn for elliptical maps, draw it with default
        # values.
        if not self._mapboundarydrawn or self._mapboundarydrawn not in ax.patches:
            ...
            if ((self.projection in ['ortho', 'geos', 'nsper', 'aeqd'] and
                ...
                limb2 = self.drawmapboundary(
                    fill_color='none', linewidth=0, ax=ax)
    ...
    def imshow(self, *args, **kwargs):
        ...
        """
        ax, plt = self._ax_plt_from_kw(kwargs)
        kwargs['extent'] = (self.llcrnrx, self.urcrnrx,
                            self.llcrnry, self.urcrnry)
    ...
    def pcolormesh(self, x, y, data, **kwargs):
        ...
        if ((np.any(x > 1e20) or np.any(y > 1e20)) and
            ...
            if nx < data.shape[0] or ny < data.shape[1]:
                raise ValueError(
                    'pcolormesh: data dimension needs to be at least that of x and y.')
            mask = (
                (x[:-1, :-1] > 1e20) |
                (x[1:, :-1] > 1e20) |
                (x[:-1, 1:] > 1e20) |
    ...
    def contour(self, x, y, data, *args, **kwargs):
        ...
        try:
            if kwargs.pop('tri', False):
                ...
                xymask = \
                    np.logical_or(np.greater(x, self.xmax + epsx),
                                  np.greater(y, self.ymax + epsy))
                xymask = xymask + \
                    np.logical_or(np.less(x, self.xmin - epsx),
                                  np.less(y, self.ymin - epsy))
    ...
    def drawlsmask(
        ...
        if lsmask is None or lsmask_lons is None or lsmask_lats is None:
            ...
            if lakes:
                lsmask = np.where(lsmask == 2, np.array(0, np.uint8), lsmask)
        ...
        if self.lsmask is None:
            ...
            if self.projection == 'cyl':
                ...
                dx = (np.pi / 180.) * \
                    (lsmask_lons[1] - lsmask_lons[0]) * self.rmajor
        ...
        # to a rectangular map projection grid.
            mask, x, y = self.transform_scalar(
                lsmask, lsmask_lons, lsmask_lats, nx, ny, returnxy=True, order=0, masked=255)
            lsmask_lats.dtype
    ...
    def warpimage(self, image="bluemarble", scale=None, **kwargs):
        ...
        if self.celestial:
            raise ValueError(
                "warpimage does not work in celestial coordinates")
        ax = kwargs.pop('ax', None) or self._check_ax()
        ...
        if not hasattr(self, '_bm_rgba') or newfile:
            ...
            nlons = self._bm_rgba.shape[1]
            nlats = self._bm_rgba.shape[0]
        ...
        if not bmproj:  # interpolation necessary.
            if newfile or not hasattr(self, '_bm_rgba_warped'):
                ...
                # sinusoidal.
                    ...
                    x1 = np.array(
                        ny * [0.5 * (self.xmax + self.xmin)], np.float64)
    ...
    def arcgisimage(self, server="http://server.arcgisonline.com/ArcGIS",
        ...
        if self.projection in _cylproj:
            dateline = _geoslib.Point(
                self(180., 0.5 * (self.llcrnrlat + self.urcrnrlat)))
        ...
        if cachedir is not None:
            ...
            filename = "%s-bbox-%s-%s-%s-%s-bboxsr%s-imagesr%s-size-%s-%s-dpi%s.png" % \
                (service, xmin, ymin, xmax, ymax, self.epsg,
                 self.epsg, xpixels, ypixels, dpi)
    ...
    def drawmapscale(self, lon, lat, lon0, lat0, length, barstyle='simple',
        ...
        if labelstyle == 'simple':
            ...
            labelstr = units + \
                " (scale factor %4.2f at %s)" % (scalefact, lonlatstr)
        ...
        if barstyle == 'fancy':
            ...
            rets.append(self.plot([x1, x4], [ytop, ytop],
                        color=linecolor, linewidth=linewidth)[0])
            # plot bottom line
            rets.append(self.plot([x1, x4], [ybottom, ybottom],
                        color=linecolor, linewidth=linewidth)[0])
            # plot left edge
            rets.append(self.plot([x1, x1], [ybottom, ytop],
                        color=linecolor, linewidth=linewidth)[0])
            # plot right edge
            rets.append(self.plot([x4, x4], [ybottom, ytop],
                        color=linecolor, linewidth=linewidth)[0])
            ...
            rets.append(self.plot([x1, x1], [ytick, ybottom],
                        color=linecolor, linewidth=linewidth)[0])
            rets.append(self.plot([xc, xc], [ytick, ybottom],
                        color=linecolor, linewidth=linewidth)[0])
            rets.append(self.plot([x4, x4], [ytick, ybottom],
                        color=linecolor, linewidth=linewidth)[0])
            # label 3 tick marks
        ...
        # 'simple' style
            ...
            rets.append(self.plot([x1, x4], [yc, yc],
                        color=linecolor, linewidth=linewidth)[0])
            rets.append(self.plot([x1, x1], [yc - yoffset, yc +
                        yoffset], color=linecolor, linewidth=linewidth)[0])
            rets.append(self.plot([x4, x4], [yc - yoffset, yc +
                        yoffset], color=linecolor, linewidth=linewidth)[0])
    ...
    def nightshade(self, date, color="k", delta=0.25, alpha=0.5, ax=None, zorder=2):
        ...
        if date.utcoffset():
            raise ValueError(
                'datetime instance must be UTC, not {0}'.format(date.tzname()))
        # get current axes instance (if none specified).
        ax = ax or self._check_ax()
        # create grid of day=0, night=1
        lons, lats, daynight = daynight_grid(
            date, delta, self.lonmin, self.lonmax)
        x, y = self(lons, lats)
        # contour the day-night grid, coloring the night area
        # with the specified color and transparency.
        CS = self.contourf(x, y, daynight, 1, colors=[
                           color], alpha=alpha, ax=ax)
        if isinstance(CS, Artist):
            # Since MPL 3.8, `QuadContourSet` objects are `Artist` objects too.
    ...
    def shiftdata(self, lonsin, datain=None, lon_0=None, fix_wrap_around=True):
        ...
        if lonsin.ndim == 2:
            ...
            if nlons > 1:
                ...
                thresh = 360. - \
                    londiff_sort[-2] if nlons > 2 else 360. - londiff_sort[-1]
...
def _choosecorners(width, height, **kwargs):
    ...
    if llcrnrlon > 1.e20 or urcrnrlon > 1.e20:
        raise ValueError(
            'width and/or height too large for this projection, try smaller values')
    else:
        return corners
...
def maskoceans(lonsin, latsin, datain, inlands=True, resolution='l', grid=5):
    ...
    lsmasko = interp(lsmask, lsmask_lons, lsmask_lats,
                     lonsin, latsin, masked=True, order=0)
    # mask input data.
    mask = lsmasko == 0
...
def _readlsmask(lakes=True, resolution='l', grid=5):
    ...
    delta = 360. / nlons
    lsmask_lons = np.linspace(-180 + 0.5 * delta,
                              180 - 0.5 * delta, nlons).astype(np.float32)
    lsmask_lats = np.linspace(-90 + 0.5 * delta, 90 -
                              0.5 * delta, nlats).astype(np.float32)
